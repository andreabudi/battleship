...
- Landscape reflows the chrome, not just the grids: the title and status share one line, the battle log sits beside the New game button below 600px of height, and during placement the picker and the controls take two columns to the right of the single board (below 360px of height the placement hint is dropped). In portrait phones the picker sits beside the board and the controls go under it. Screenshot both orientations, since a layout that merely does not overflow can still leave the boards unusably small.
